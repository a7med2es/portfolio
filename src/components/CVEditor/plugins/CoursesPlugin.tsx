...
import { Command, CourseBlock, UUID } from '../types';
...
export const CoursesPlugin: React.FC = () => {
  ...
  const handleUpdate = (block: CourseBlock, field: keyof CourseBlock, value: any) => {
    executeCommand({
      type: 'UPDATE_BLOCK',
      section: 'courses',
      blockId: block.id,
  ...
  return (
    <div className="border border-slate-200 rounded-lg bg-white mb-4 overflow-hidden">
      ...
      >
        <h3 className="font-bold text-slate-800 text-sm tracking-wide">Training & Certificates</h3>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div className="font-semibold text-sm text-slate-700">
                    {block.title || '(No Title)'} <span className="font-normal text-slate-400 text-xs">at {block.institution}</span>
                  </div>
                </div>
                <button 
              ...
              {expandedId === block.id && (
                <div className="p-3 pt-0 space-y-3 border-t border-slate-100">
                  <div className="grid grid-cols-2 gap-3 mt-3">
                    <div>
                      <label className="block text-xs text-slate-500 mb-1">Course Title</label>
                      <input 
                        type="text" 
                        value={block.title} 
                    ...
                    <div>
                      <label className="block text-xs text-slate-500 mb-1">Certificate Link</label>
                      <input 
                        type="text" 
                        value={block.certificate_link || ''} 
                        onChange={e => handleUpdate(block, 'certificate_link', e.target.value)}
                        className="w-full border border-slate-200 p-1.5 rounded text-sm focus:outline-blue-500"
                      />
                    </div>
                  </div>
